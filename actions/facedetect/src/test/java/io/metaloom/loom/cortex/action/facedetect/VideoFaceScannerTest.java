...
public class VideoFaceScannerTest {
	...
	private static final int WINDOW_COUNT = 30;
	private static final int WINDOW_SIZE = 120;
	private static final int WINDOW_STEPS = 15;
	...
	public void testExampleCode() throws InterruptedException, IOException {
		try (VideoFile video = Videos.open("/extra/vid/4.mkv")) {
			...
			VideoFaceScannerReport report = detector.scan(video, WINDOW_COUNT, WINDOW_SIZE, WINDOW_STEPS);
			long dur = System.currentTimeMillis() - start;
			System.out.println("Scan took " + dur + " ms / " + (dur / 1000) + " s");
			// for (Face face : report.getFaces()) {
			// System.out.println("Face [" + face.get("blurriness") + "][" + face.get("frame") + "] " + Arrays.toString(face.getEmbedding()));
